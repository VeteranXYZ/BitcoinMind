    ...
    async function fetchHashRate() {
      try {
        ...
        if (data?.currentHashrate) {
          setHashRate(data.currentHashrate);
        }
      } catch {}
    }
    fetchHashRate();
    const interval = setInterval(fetchHashRate, 120000);
    return () => { cancelled = true; clearInterval(interval); };
  }, []);
  ...
  useEffect(() => {
    ...
    async function fetchMempool() {
      try {
        ...
        if (cancelled) return;
        if (data?.count != null) {
          setMempoolCount(data.count);
        }
      } catch {}
    }
    fetchMempool();
    const interval = setInterval(fetchMempool, 60000);
    return () => { cancelled = true; clearInterval(interval); };
  }, []);

  // Fetch node count (once) — bitnodes API may be CORS-blocked from browser,
  // so this stat gracefully disappears if the fetch fails.
  useEffect(() => {
    let cancelled = false;
    async function fetchNodes() {
      try {
        ...
        // CORS or network error — stat simply won't render
      }
    }
    fetchNodes();
