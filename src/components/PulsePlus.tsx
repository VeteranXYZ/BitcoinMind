...
export default function PulsePlus({ initialSnapshot }: Props) {
  const initialHeight = Number(initialSnapshot?.height);
  const [height, setHeight] = useState<number>(
    Number.isFinite(initialHeight) && initialHeight > 0 ? initialHeight : FALLBACK_BLOCK_HEIGHT,
  );
  const [pulsed, setPulsed] = useState(false);
  const [now, setNow] = useState(Date.now());
  const [hashRate, setHashRate] = useState<number | null>(
    Number.isFinite(initialSnapshot?.hashRate) ? initialSnapshot?.hashRate ?? null : null,
  );
  const [mempoolCount, setMempoolCount] = useState<number | null>(
    Number.isFinite(initialSnapshot?.mempoolCount) ? initialSnapshot?.mempoolCount ?? null : null,
  );
  const [snapshotDate, setSnapshotDate] = useState(initialSnapshot?.fetchedAt);
  ...
  useEffect(() => {
    ...
    async function poll() {
      try {
        ...
        if (data.fetchedAt) setSnapshotDate(data.fetchedAt);
      } catch {
        // Keep the generated snapshot visible when a refresh is unavailable.
      }
    }
  ...
  return (
    <section class="pulse" aria-label="Bitcoin network clock">
      <div class="pulse-hd">
        ...
        <span class={`pulse-live ${pulsed ? 'pulse-live--flash' : ''}`}>
